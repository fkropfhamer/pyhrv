...
class PDFReport:
	...
	def __init__(self, signal=None, nni=None, rpeaks=None, results=None, sampling_rate=None):
		...
		self.hrv_keys = _pyhrv.utils.load_hrv_keys_json()

		# Get the NNI series
		if self.signal is not None:
			rpeaks = ecg(self.signal, self.sampling_rate, show=False)[2]
			self.nni = _pyhrv.utils.check_input(None, rpeaks)
		else:
			# Check input data series
			if nni is not None or rpeaks is not None:
				self.nni = _pyhrv.utils.check_input(nni, rpeaks)
	...
	def set_general_info(self, subject=None, experiment=None, age=None, gender=None, comment=None):
		...
		"""
		self._general_info_latex['version'] = self._new_command('version', str(_pyhrv.__version__), 'r')
	...
	def create_report(self, fname=None, output_path=None, terminal_output=False):
		...
		if fname is None:
			...
			while error:
				try:
					fname = _pyhrv.utils.check_fname(os.path.join(self._build, '%s.pdf' % fname), 'pdf')[1][1]
					error = False
				except:
					error = True
	...
	def clear(self):
		...
		for file in os.listdir(self._path):
			if file not in ['__init__.py', 'main.tex', 'parameters.tex', '_pyhrv.png', 'pdf_report.py', 'settings.tex']:
	...
	def _prepare_figures(self):
		...
		if isinstance(self._general_info['age'], int) and isinstance(self._general_info['gender'], str):
			try:
				plots['hr_heatplot'] = \
				_pyhrv.utils.heart_rate_heatplot(nni=self.nni, show=False, age=self._general_info['age'],
...
def hrv_report(results=None,
	...
	rfile, _ = _pyhrv.utils.check_fname(path, file_format, rfile)

	if 'hrv_export' in str(path):
		path = path.replace('hrv_export', 'hrv_report')

	# Load HRV parameters metadata
	params = _pyhrv.utils.load_hrv_keys_json()
