...
def _check_limits(interval, name):
    ...
    if interval[0] > interval[1]:
        ...
        warnings.warn(
            "The provided lower limit of the parameter '%s' is greater than the upper limit. Limits have been switched." % name)
    if interval[0] == interval[1]:
        raise ValueError(
            "The provided lower and upper limits of the parameter '%s' are invalid as they are identical." % name)
    return interval
...
def segmentation(nni=None, full=True, duration=300, warn=True):
    ...
    if np.sum(nni) > duration:
        ...
        if warn:
            warnings.warn(
                "Signal duration is to short for segmentation into %is. Input data will be returned." % duration)
        return nni, False
